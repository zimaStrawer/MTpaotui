...
> 产品业务与交互规则见 `product.md`；研发实现见 `tech.md`。
...
美团跑腿代办重设计的整体气质，不是冷静的 B2B 工具，而是一款**用美团黄把“找人跑腿”这件看不见的事变成“看得见、找得到、信得过”的亲切移动服务**。首屏一打开，整片黄色顶带上站着美团的黄色骑手吉祥物，“一小时全城送”的口号直接给出速度承诺；下方一张白色大圆角卡片把下单表单托出来，像从黄色背景里“浮”出来的一层。整个页面密度不低，但层级清晰——黄色负责品牌和情绪，白色卡片负责任务，黑色/灰色文字负责信息，红色/绿色标签负责状态和信任。
...
### ② 标志性视觉手法 · Signature Moves

这套设计的招牌手法可以概括为：**“用美团黄顶带和吉祥物建立即时品牌感，再用白色大圆角卡片把复杂的委托流程装成一连串可滑动、可点击、可追踪的任务层”**。

- **黄色顶带 + 吉祥物 Hero**：首屏和核心页顶部的 `{colors.brand-primary}` 黄色背景带上放置骑手吉祥物与 headline（如“一小时全城送”），是最大面积的品牌识别。它不是按钮，而是“情绪背景”，让用户 0.3 秒内知道这是美团跑腿。
- **白色大圆角卡片浮于黄色**：主表单、地址、物品、价格、订单信息都用 `{rounded.16}` 或更大的白色卡片承载，背景 `{colors.bg-container}`，与黄色顶带形成强对比。这是把“任务”从“品牌背景”里托出来的核心构图。
- **每任务屏只有一个黄色主 CTA**：无论“去下单”“保存并使用”“确定”“提交订单”，每屏只留一个黄色胶囊主按钮，避免在已经很黄的顶带里再出现多个竞争焦点。
- **骑手身份卡 + 地图**（人可信）：订单追踪页用地图上的骑手小车 + 底部骑手信息卡（头像、姓名、“美团跑腿”徽章、满意度、评分、电话/聊天）把“谁在帮我送”可视化。这是“人可信”最具体的设计表达。
- **取/收进度时间轴**（事可感）：用横向节点时间轴（取 → 收）配合黄色已完成段，把“到哪了”变成始终可见的进度脊。截图中的“21:07-21:12 预计取件”即这条脊的文字落点。
- **绿色保价服务作为独立信任色**：`{colors.insurance-primary}` 绿色专门用于“省心送”“保价服务”等信任模块，与黄色品牌色、红色促销标签职责分开，避免功能色被品牌色淹没。
- **红色/橙色只用于促销与紧急**：`{colors.alert}` 和 `{colors.accent-primary}` 只出现在“最高减20元”“限时 7.5 折”“1对1急送 权益升级”等需要用户立即注意的节点，不是品牌主色。
- **价格用大号数字突出显示**：SF Pro / 等宽数字风格的价格（¥14.5、¥4.9、¥9.3）在卡片中单独成区，方便用户一眼比较。
- **深色交易底栏**：订单确认页底部用 `{colors.bg-black}` 深色条固定价格 + 黄色“提交订单”按钮，把“付钱”这一动作从内容区独立出来，形成稳定的心理锚点。
...
- AppShell、固定栏容器和安全区的代码组织属于 `tech.md`，不在本文件中规定实现方式。
