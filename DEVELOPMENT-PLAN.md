...
1. **每页开工流程**:需求方给该页具体信息(含该页用到的组件及其**变体**)→ MCP 拉对应 frame(node id 见 §6 表)→ 实现 → 按验收标准过检。
2. **数据层 / mock 随页面演进**:页面开发中发现模型缺字段、mock 缺数据时,由开发侧判断并直接调整,commit 信息说明改动原因;需求方无需关心这一层。
3. **Commit 纪律**:小步递进,一个关切一个 commit(如 `feat(tokens): …`、`feat(item-info): 分段状态机`);禁止巨型单提交。
4. **不做清单**(§14)随时对照:无 A/B、无失败态、无埋点实现、无真后端。
