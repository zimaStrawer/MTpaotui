...
> 配套文档:工程规范见 `MTprototype-tech-spec.md`(定位、技术栈、领域模型、约束均以它为准)。
> 本计划按 §13 构建顺序展开;每页的具体设计信息在**开工该页时**由需求方提供,配合 Figma MCP 现拉对应 frame。
> 优先级仲裁(§11):**P0 流程正确 > P1 时间轴稳定 > P2 手感 > P3 视觉还原 > P4 代码干净**。
...
- [ ] 目录结构按 §2 建齐:`app / pages / components / design-tokens / data(models·repositories·mock) / store / lib`
...
- [ ] **共享组件**(§6 清单,纯 props 受控):`StatusBar`、`NavigationBar`、`TabBar`、`HomeIndicator`、业务选择、地址填写-居中、物品类型选择、物品体积、保价、进度条、骑手卡片、物品凭证、取收标签、车型推荐提示条

**验收**:组件在演示路由渲染正常;色值/间距/圆角零魔法数字。
...
- [ ] 压缩时间轴 mock:每 10–15 秒自动推进,`accepting → accepted → picked → delivering → arrived → completed`
- [ ] 收货码:`picked` 前 ✱✱✱✱,之后揭示 4 位
- [ ] 信任三角全量呈现:骑手卡片+徽章(人可信)、物品凭证+照片位(物可见)、进度条+时间轴+情境提示(事可感)
- [ ] 终态「订单已完成」(本次服务 XX 米、XX 分钟)
- [ ] 推进逻辑住 mock 层/store,不进组件;刷新/重开 = 干净起点

**验收**:下单后不动手,一分钟内看完整条弧线,顺序与文案稳定复现。
...
1. **每页开工流程**:需求方给该页具体信息 → MCP 拉对应 frame(node id 见 §6 表)→ 实现 → 按验收标准过检。
2. **Commit 纪律**:小步递进,一个关切一个 commit(如 `feat(tokens): …`、`feat(item-info): 分段状态机`);禁止巨型单提交。
3. **不做清单**(§14)随时对照:无 A/B、无失败态、无埋点实现、无真后端。
